...
def read_complist(file_path):
    cfiles = set()
    with open(file_path, 'r') as f:
        for line in f.readlines():
            cfiles.add(line.strip())
    return cfiles
...
def main():
    ...
    parser.add_argument(
        "-C",
        "--compile-file-list",
        help='Get list of sources from a prepared list',
    )
    ...
    if args.debug_sources_file:
        ...
        logging.info("Total compiled files %d", len(compiled_files))
    if args.compile_file_list:
        compiled_files = read_complist(args.compile_file_list)
        logging.info("Total compiled files %d", len(compiled_files))
